...
export default function CalendarPage() {
  ...
  const todayKey = useMemo(() => toDateKeyInTimeZone(new Date(), timeZone), [timeZone]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setError(null);
        const data = await apiFetchJson<SessionListItem[]>(`/teacher/sessions${query}`);
      ...
      {sessions === null ? (
        ...
        <div className="stack" data-testid="session-list">
          {filteredSessions.map((session) => (
            <div key={session.id} className="card" data-testid="session-item">
              <div className="row" style={{ justifyContent: 'space-between' }}>
