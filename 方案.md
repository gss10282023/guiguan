...
* `TeacherStudentRate`：
  * `teacherId`, `studentId`, `hourlyRateCents`, `currency`（例如 AUD/CNY）
  * **唯一约束**：(teacherId, studentId)
* `Session`（每节课）：
  * `teacherId`, `studentId`
  * `startAtUtc`, `endAtUtc`, `classTimeZone`
  * `status`：SCHEDULED/CANCELLED/COMPLETED
  * `consumesUnits`（默认 1）
  * `rateCentsSnapshot`, `currencySnapshot`
  * `createdByAdminId`
...
> * PUT /admin/rates 设置/更新 teacher-student hourlyRateCents + currency（upsert）
...
* `POST /admin/sessions` 创建课程：
  ...
  * 从 `TeacherStudentRate` 读取费率并写入 `rateCentsSnapshot`
  * 冲突检测（至少老师不冲突）：
...
> 实现 Session 排课：
>
> * POST /admin/sessions：输入 teacherId, studentId, startAtUtc, endAtUtc, classTimeZone, consumesUnits(默认1)。创建时从 TeacherStudentRate 读取费率快照写入 session。校验老师时间不冲突（overlap 检测）。写入 AuditLog。
...
* 金额 = sum( (durationMinutes/60) \* rateCentsSnapshot )
